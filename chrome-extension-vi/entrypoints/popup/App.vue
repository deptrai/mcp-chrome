  ...
  <div class="popup-container">
    <div class="header">
      <div class="header-content">
        <h1 class="header-title">Chrome MCP Server - Tiếng Việt</h1>
      </div>
    </div>
    <div class="content">
      <div class="section">
        <h2 class="section-title">Cấu hình Native Server</h2>
        <div class="config-card">
          <div class="status-section">
            <div class="status-header">
              <p class="status-label">Trạng thái hoạt động</p>
              <button class="refresh-status-button" @click="refreshServerStatus" title="Làm mới trạng thái">
                🔄
              </button>
            </div>
            ...
            <div v-if="serverStatus.lastUpdated" class="status-timestamp">
              Cập nhật lần cuối: {{ new Date(serverStatus.lastUpdated).toLocaleTimeString() }}
            </div>
          </div>

          <div v-if="showMcpConfig" class="mcp-config-section">
            <div class="mcp-config-header">
              <p class="mcp-config-label">Cấu hình MCP Server</p>
          ...
          <div class="port-section">
            <label for="port" class="port-label">Cổng kết nối</label>
            <input
              type="text"
              id="port"
          ...
          <button class="connect-button" :disabled="isConnecting" @click="testNativeConnection">
            <BoltIcon />
            <span>{{
              isConnecting ? 'Đang kết nối...' : nativeConnectionStatus === 'connected' ? 'Ngắt kết nối' : 'Kết nối'
            }}</span>
          </button>
        </div>
      </div>
      ...
      <div class="section">
        <h2 class="section-title">Công cụ Ngữ nghĩa</h2>
        <div class="semantic-engine-card">
          <div class="semantic-engine-status">
            ...
            <div v-if="semanticEngineLastUpdated" class="status-timestamp">
              Cập nhật lần cuối: {{ new Date(semanticEngineLastUpdated).toLocaleTimeString() }}
            </div>
          </div>
      ...
      <div class="section">
        <h2 class="section-title">Mô hình Embedding</h2>
        ...
        <div v-if="modelInitializationStatus === 'error'" class="error-card">
          <div class="error-content">
            ...
            <div class="error-details">
              <p class="error-title">Khởi tạo mô hình thất bại</p>
              <p class="error-message">{{ modelErrorMessage || 'Tải mô hình thất bại' }}</p>
      ...
      <div class="section">
        ...
        >
          <TrashIcon />
          <span>{{ isClearingData ? 'Đang xóa...' : 'Xóa tất cả dữ liệu' }}</span>
        </button>
      </div>
    ...
    <div class="footer">
      <p class="footer-text">Chrome MCP Server cho AI - Phiên bản tiếng Việt</p>
    </div>

    <ConfirmDialog
      :visible="showClearConfirmation"
      title="Xác nhận xóa dữ liệu"
      message="Thao tác này sẽ xóa tất cả nội dung trang web đã lập chỉ mục và dữ liệu vector, bao gồm:"
      :items="['Chỉ mục nội dung văn bản của tất cả trang web', 'Dữ liệu vector embedding', 'Lịch sử tìm kiếm và cache']"
      warning="Thao tác này không thể hoàn tác! Sau khi xóa, bạn cần duyệt lại các trang web để xây dựng lại chỉ mục."
      icon="⚠️"
      confirm-text="Xác nhận xóa"
      cancel-text="Hủy"
      confirming-text="Đang xóa..."
...
const copyButtonText = ref('Sao chép cấu hình');
...
const getStatusText = () => {
  if (nativeConnectionStatus.value === 'connected') {
    if (serverStatus.value.isRunning) {
      return `Dịch vụ đang chạy (Cổng: ${serverStatus.value.port || 'Không xác định'})`;
    } else {
      return 'Đã kết nối, dịch vụ chưa khởi động';
    }
  } else if (nativeConnectionStatus.value === 'disconnected') {
    return 'Dịch vụ chưa kết nối';
  } else {
...
const getSemanticEngineStatusText = () => {
  switch (semanticEngineStatus.value) {
    case 'ready':
      return 'Công cụ ngữ nghĩa đã sẵn sàng';
    case 'initializing':
      return 'Đang khởi tạo công cụ ngữ nghĩa...';
    case 'error':
      return 'Khởi tạo công cụ ngữ nghĩa thất bại';
    case 'idle':
    default:
      return 'Công cụ ngữ nghĩa chưa được khởi tạo';
  }
};
...
const getProgressText = () => {
  if (isModelDownloading.value) {
    return `Đang tải mô hình... ${modelDownloadProgress.value}%`;
  } else if (isModelSwitching.value) {
    return modelSwitchProgress.value || 'Đang chuyển đổi mô hình...';
  }
  return '';
};
...
const getErrorTypeText = () => {
  switch (modelErrorType.value) {
    case 'network':
      return 'Lỗi kết nối mạng, vui lòng kiểm tra kết nối mạng và thử lại';
    case 'file':
      return 'File mô hình bị hỏng hoặc không đầy đủ, vui lòng thử tải lại';
    case 'unknown':
    default:
      return 'Lỗi không xác định, vui lòng kiểm tra mạng có thể truy cập Hugging Face';
  }
};
...
const getSemanticEngineButtonText = () => {
  switch (semanticEngineStatus.value) {
    ...
    case 'idle':
    default:
      return 'Khởi tạo công cụ ngữ nghĩa';
  }
};
